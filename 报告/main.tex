...
\begin{align}
    \hat{r}_e &\le T_{\text{edge}}, \quad \forall e \in \pi &&\text{(单步动作风险约束)} \\
...
\begin{enumerate}
    \item \textbf{局部地形特征 \(X_{\text{map}}^{s,a} \in \mathbb{R}^{31 \times 31 \times 6}\)}：以车辆当前位置为中心、沿车身航向旋转至车辆坐标系的局部地图切片。上标 \((s, a)\) 表示切片的提取参数：状态 \(s\) 决定切片中心位置与旋转角度，动作 \(a\) 决定第6通道（扫掠掩码）的形状。6个通道均为几何特征：
    \begin{enumerate}
        \item 相对高程 \(Z\)（以车辆中心高度归零）；
        \item 纵向坡度 \(G_u\)（车头方向梯度，影响俯仰）；
        \item 横向坡度 \(G_v\)（垂直车头方向梯度，影响侧翻）；
        \item 局部粗糙度 \(R\)（邻域窗口内高度的标准差，衡量地表颠簸程度）；
        \item 车身掩码 \(M_{\text{body}}\)（车辆当前 footprint 的二值图）；
        \item 动作扫掠掩码 \(M_{\text{swept}}\)（车身沿动作轨迹扫过区域的二值图，共15个时刻的并集）。
    \end{enumerate}
    \item \textbf{车辆参数向量 \(\theta_v \in \mathbb{R}^{12}\)}：包含车长 \(L\)、车宽 \(W\)、轴距 \(l\)、轮距 \(b\)、轮径 \(r_w\)、底盘离地间隙 \(c_g\)、质量 \(m\)、质心高度 \(z_c\)、最大安全横滚角 \(\phi_{\max}\)、最大安全俯仰角 \(\theta_{\max}\)、最大爬坡角 \(\alpha_{\max}\)、最大驱动力 \(F_{\max}\)。
    \item \textbf{动作参数 \(a\)} 与 \textbf{摩擦系数 \(\mu\)}：\(a = [\Delta d, \Delta \psi]\) 为当前候选动作；\(\mu\) 为地面摩擦系数（标量，非6通道之一，表示接触面材质属性：干地约0.7--0.9，草地约0.4--0.6，湿泥约0.2--0.4）。注意粗糙度 \(R\) 描述的是地形几何起伏，与摩擦系数 \(\mu\)（材质滑腻程度）是独立的物理量。
\end{enumerate}
...
\textbf{1. 突破规则启发函数的能力上限——机器学习赋能搜索。}传统2.5D A*仅利用几何高度差作为代价，本质上属于静态惩罚。然而地形对车辆造成的侧翻、打滑等风险是高度非线性的，取决于车辆的轴距 \(l\)、轮距 \(b\)、底盘高度 \(c_g\) 等物理参数与当前姿态的组合。外层引入经 PSO 优化的 ML 模型后，搜索树的扩展获得了物理引擎级别的先验知识。对于超过最优阈值 \(T^*_{\text{edge}}\) 的候选边，A*可提前剪枝，从而避免传统搜索算到死胡同才回溯的维度灾难。针对 ML 在分布外数据上可能失效的问题，本方案在耦合边界处增加了 Geometric Guard 手工几何保底机制，保证了在极端未见地形中搜索代价的下界安全性。
