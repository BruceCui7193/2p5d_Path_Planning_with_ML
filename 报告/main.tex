...
\begin{itemize}
    ...
    \item \textbf{动作空间 \(\mathcal{A}\)}：定义为一组局部运动基元 \(a = [\Delta d, \Delta \psi]\)，其中 \(\Delta d\) 为沿当前航向的弧长位移量（0或0.45\,m），\(\Delta \psi\) 为航向角变化量（\(0\) 或 \(\pm 22.5^{\circ}\)），两者组合构成直行与不同曲率的转向动作。此处的 \(\Delta d\) 是位移距离，与状态变量 \(s\) 无关。
...
\begin{enumerate}
    \item \textbf{局部地形特征 \(X_{\text{map}}^{s,a} \in \mathbb{R}^{31 \times 31 \times 6}\)}：以车辆当前位置为中心、沿车身航向旋转至车辆坐标系的局部地图切片。上标 \((s, a)\) 表示状态 \(s\) 决定切片中心与朝向，动作 \(a\) 决定扫掠掩码形状。6个通道依次为：①相对高程 \(Z\)，②纵向坡度 \(G_u\)（影响俯仰），③横向坡度 \(G_v\)（影响侧翻），④局部粗糙度 \(R\)（高度标准差，衡量颠簸），⑤车身掩码 \(M_{\text{body}}\)，⑥动作扫掠掩码 \(M_{\text{swept}}\)（15个时刻车身扫过区域的并集）。
